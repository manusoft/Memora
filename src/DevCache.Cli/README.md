...
- Later: history, tab completion, pipe support, scripting mode (`devcache-cli -f script.txt`)

### Project Setup
Create a new console project inside your solution:

```bash
dotnet new console -n DevCache.Cli
dotnet sln add DevCache.Cli/DevCache.Cli.csproj
```
...
```bash
cd DevCache.Cli
dotnet add reference ../DevCache.Core/DevCache.Core.csproj
```
...
using DevCache; // ← from DevCache.Core (RespReader, RespWriter, RespValue)

namespace DevCache.Cli
{
    class Program
    {
